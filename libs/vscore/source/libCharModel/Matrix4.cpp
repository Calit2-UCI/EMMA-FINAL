...
float Matrix4::getElement( int i, int j ) const
{
	#ifdef _DEBUG
...
void Matrix4::setElement( int i, int j, float x )
{
	#ifdef _DEBUG
	assert( i >= 0 || i <= 3 || j >= 0 || j <= 3 );
	#endif
...
Matrix4& Matrix4::invert()
{
	...
	for( int i = 0; i < 4; ++i )
	{
		...
		{
			sgn = 1 - ( ( i + j ) % 2 ) * 2;
			getSubMatrix4( i, j, &sub_mat[0][0] );
			t_mat.setElement( j, i, static_cast<float>(sgn) * determinant3( &sub_mat[0][0] ) / det );
		}
	}
...
{
	if( !V_EQF( m[0][0], mat.m[0][0] ) ||
		!V_EQF( m[0][1], mat.m[0][1] ) ||
		!V_EQF( m[0][2], mat.m[0][2] ) ||
		!V_EQF( m[0][3], mat.m[0][3] ) ||
		!V_EQF( m[1][0], mat.m[1][0] ) ||
		!V_EQF( m[1][1], mat.m[1][1] ) ||
		!V_EQF( m[1][2], mat.m[1][2] ) ||
		!V_EQF( m[1][3], mat.m[1][3] ) ||
		!V_EQF( m[2][0], mat.m[2][0] ) ||
		!V_EQF( m[2][1], mat.m[2][1] ) ||
		!V_EQF( m[2][2], mat.m[2][2] ) ||
		!V_EQF( m[2][3], mat.m[2][3] ) ||
		!V_EQF( m[3][0], mat.m[3][0] ) ||
		!V_EQF( m[3][1], mat.m[3][1] ) ||
		!V_EQF( m[3][2], mat.m[3][2] ) ||
		!V_EQF( m[3][3], mat.m[3][3] ) )
		return false;
...
Vector3 Matrix4::operator*( const Vector3& v ) const
{
	...
	res.w = m[3][0] * v.x + m[3][1] * v.y + m[3][2] * v.z + m[3][3];
...
void Matrix4::getSubMatrix4( int i, int j, float* subMat ) const
{
	int di, dj, si, sj;
